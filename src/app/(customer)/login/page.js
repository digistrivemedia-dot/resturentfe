...
export default function LoginPage() {
  ...
  const { sendOtp, googleLogin, isLoading, isAuthenticated, user } = useAuthStore();
  ...
  const triggerGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        const { user: loggedUser, isNewUser } = await googleLogin(tokenResponse.access_token);
        toast.success("Logged in with Google!");
        if (isNewUser && !loggedUser.name) {
          router.push("/complete-profile");
        } else if (loggedUser.role === "restaurant_owner") {
  ...
  const handleGoogleLogin = () => triggerGoogleLogin();
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-10 lg:p-16 bg-white">
        ...
        <div className="w-full max-w-[360px]">
          ...
          <button
            ...
            className="w-full h-12 flex items-center justify-center gap-3 border border-border-default rounded-[var(--radius-lg)] text-sm font-semibold text-text-primary hover:bg-bg-hover transition-colors mb-6 shadow-[var(--shadow-sm)] disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {googleLoading ? (
